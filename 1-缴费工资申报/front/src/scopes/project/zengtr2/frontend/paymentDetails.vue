					...
					>
						...
						<ta-form-item label="缴费基数总额" :span="6"
									  ...
									  :fieldDecoratorOptions="{rules: [{required:true,message:'请输入缴费基数总额'},
											 { pattern: /^[0-9]+([.][0-9]{1,2})?$/ , message: '基数格式不正确' },
											 {validator: checkPaymentBase}]}"
						>
		...
		fnSave () {
			...
			this.Base.submit(null, {
				...
				for (let i = 0; i < this.validatedList.length; i++) {
					...
					for (let j = 0; j < this.clctInfoList.length; j++) {
						...
						if (temp2.accrym !== null && temp1.insutype === temp2.insutype) {
							if (temp2.clctFlag === 1 || (parseInt(temp2.accrym) < parseInt(temp1.endYM)
								&& parseInt(temp2.accrym) > parseInt(temp1.startYM))) {
								//存在记录
								flag = 1
								...
								setTimeout(() => {
									this.$message.error(tempinsutype + '险种' + temp2.accrym +
										'年月已存在缴费记录，不能补录！')
									console.log(tempinsutype + '险种' + temp2.accrym +
										'年月已存在缴费记录，不能补录！')
								}, 100)
							}
						} else {
				...
				for (let i = 0; i < this.validatedList.length; i++) {
					...
					let year = (parseInt((temp.endYM).substring(0, 4)) -
						parseInt((temp.startYM).substring(0, 4))) * 12
					let month = parseInt((temp.endYM).substring(4)) -
						parseInt((temp.startYM).substring(4))
		...
		fnAdd () {
			...
			if (this.checkedInfoInsuList.length === 0) {
				...
				if (this.checkedInfoInsuList[0].fstInsuYm !== null) {
					console.log('校验日期')
					let startYM = this.paymentBaseForm.getFieldMomentValue('startYM').substring(0, 4) +
						this.paymentBaseForm.getFieldMomentValue('startYM').substring(5, 7)
				...
				if (this.validatedList.length !== 0) {
					...
					for (const element of this.validatedList) {
						let tempS1 = parseInt(this.paymentBaseForm.getFieldMomentValue('startYM').substring(0, 4)
							+ this.paymentBaseForm.getFieldMomentValue('startYM').substring(5, 7))
						let tempS2 = parseInt(element.startYM)
						let tempE1 = parseInt(this.paymentBaseForm.getFieldMomentValue('endYM').substring(0, 4)
							+ this.paymentBaseForm.getFieldMomentValue('endYM').substring(5, 7))
				...
				setTimeout(() => {
					...
					tempList1.forEach(item => {
						...
						Object.assign(item, {
							startYM: this.paymentBaseForm.getFieldMomentValue('startYM').substring(0, 4)
								+ this.paymentBaseForm.getFieldMomentValue('startYM').substring(5, 7)
						})
						Object.assign(item, {
							endYM: this.paymentBaseForm.getFieldMomentValue('endYM').substring(0, 4)
								+ this.paymentBaseForm.getFieldMomentValue('endYM').substring(5, 7)
						})
